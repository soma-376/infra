 ...
 *
 * 인증이 전혀 없으므로 방어선은 `DevAlbSg` 의 허용 CIDR 하나뿐이다
 * (`infra:dev-open-ingress` 경고 참조).
 */
export class DevEdgeStack extends Stack {
  ...
  /**
   * :80 리스너 - `/v1/*` 는 collector, `/api/*` 는 dashboard.
   *
  ...
  private buildAppListener(
    ...
    const otlpTargetGroup = new ApplicationTargetGroup(
      this,
      'DevCollectorTg',
      {
        vpc: props.vpc,
